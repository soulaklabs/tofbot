...
# determination of the win in the dating game
    ...
    def handle_msg(self, msg_text, _chan, _nick):
        #calcule les chances si la synthaxe de la chaine matche grande irma,chaine1<3chaine2?
        ...
        if re.match(r"(grande irma,)\s*[a-zA-Z_]+.*<3(.)*[a-zA-Z_]+.*(\?)+",chaine):
            ...
            #calcul de la somme theologique (theonum) de tous les identifiants numériques
            theonum=sum(chaine)
       
            #boucle sur theonum pour que la somme finale soit comprise entre 1 et 9
            while theonum>9:
                chaine=[int(i) for i in str(theonum)]
                theonum=sum(chaine)
            #calcul du pourcentage de l'amour calibré sur Vanessa et Erick = 100%   
            pourcentage=(11-theonum)*10
            text="   "+str(pourcentage)+ '% de réussite pour'+ amoureux1+' et '+amoureux2
